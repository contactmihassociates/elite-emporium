...
## 8. Payments — Razorpay (live, two modes)

**Status:** Both modes are wired and waiting for a Razorpay account. Pick one (or both) to turn online payments on.

### Mode A (recommended): Embedded Checkout — opens an in-page modal

1. Sign in to [Razorpay Dashboard](https://dashboard.razorpay.com/) → free, instant signup.
2. **Settings → API Keys → Generate Key.** Razorpay starts you in **Test Mode** automatically (no KYC, no fees).
3. Copy the **Key ID** — it looks like `rzp_test_AbCdEf123XYZ` (Test) or `rzp_live_…` (Live).
4. Open `script.js` at the top, set:
   ```js
   razorpayKeyId: "rzp_test_YOUR_ID_HERE",
   razorpayLiveMode: false, // flip to true after you complete KYC + first live test
   ```
5. Commit + push. Done. The **💳 Pay Online** button is now active on the cart.

What it does (`payOnline()` in `script.js`):
- Lazy-loads `https://checkout.razorpay.com/v1/checkout.js` only when the button is clicked.
- Opens the Razorpay modal **on top of your cart page** (no redirect).
- Customer pays via UPI / cards / netbanking / wallets through Razorpay's secure flow.
- On success: saves the order to local history with status `"Paid online ✅"` + the `razorpay_payment_id`, fires confetti, opens WhatsApp with a payment-confirmation message to the merchant, and redirects to `orders.html`.
- On dismiss/failure: order stays as `"Awaiting payment"` so the customer can retry. Cart is preserved.

**Test card** for Test Mode: `4111 1111 1111 1111`, any future expiry, CVV `123`.

**Going live:** complete KYC in the Razorpay dashboard (PAN, business proof, bank account). Generate a `rzp_live_…` key and swap it in. Flip `razorpayLiveMode: true`. Done.

**Important caveat (frontend-only mode):** without server-side order creation, payments succeed but `order_id` is generated by Razorpay, not by you. That means refunds and disputes are slightly harder to track. For small volumes this is fine — every payment shows up in your dashboard with the order reference in the `notes` field. When you outgrow this, add a Vercel serverless function (~30 lines) to create orders server-side; the frontend code already passes the right shape.

### Mode B (fallback): Hosted Payment Link / Payment Page

For zero-code merchants or as a backup when checkout.js fails to load:

1. In Razorpay Dashboard → **Payment Pages** → create a "Customer-amount" page.
2. Copy the public URL (e.g. `https://razorpay.com/payment-link/plink_XXXX`).
3. Paste into `CONFIG.razorpayPaymentLink` in `script.js`.

When Key ID is missing but a Payment Link is set, `payOnline()` opens the Razorpay-hosted page in a new tab with name/phone/amount/reference prefilled. Used automatically when Mode A is unconfigured.

### Both modes are frontend-only — no backend required.
